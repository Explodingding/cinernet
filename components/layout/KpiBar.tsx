import type { TopologyNode, TopologyEdge, Status } from '@/types/topology';

interface KpiBarProps {
  nodes: TopologyNode[];
  edges: TopologyEdge[];
}

interface KpiCardProps {
  label: string;
  value: number;
  accentColor: string;
  sublabel?: string;
}

function KpiCard({ label, value, accentColor, sublabel }: KpiCardProps) {
  return (
    <div
      className="flex items-center gap-3 px-4 py-2.5 rounded-lg flex-1 min-w-0"
      style={{
        background: 'rgba(15, 23, 42, 0.7)',
        border: `1px solid rgba(255,255,255,0.06)`,
        borderLeft: `3px solid ${accentColor}`,
        boxShadow: `0 0 12px rgba(0,0,0,0.3), inset 0 0 20px rgba(0,0,0,0.1)`,
      }}
    >
      <div
        className="text-3xl font-bold tabular-nums leading-none"
        style={{
          fontFamily: 'var(--font-jetbrains-mono)',
          color: accentColor,
          textShadow: `0 0 16px ${accentColor}50`,
        }}
      >
        {value}
      </div>
      <div>
        <div className="text-xs font-semibold text-slate-300 leading-tight">{label}</div>
        {sublabel && (
          <div className="text-[10px] text-slate-500 mt-0.5">{sublabel}</div>
        )}
      </div>
    </div>
  );
}

export function KpiBar({ nodes, edges }: KpiBarProps) {
  const allElements = [...nodes, ...edges];
  const counts: Record<Status, number> = {
    operational: 0,
    investigation: 0,
    fault: 0,
  };
  allElements.forEach(e => counts[e.status]++);

  return (
    <div
      className="flex items-center gap-3 px-5 py-2.5 shrink-0"
      style={{
        background: 'rgba(15, 23, 42, 0.5)',
        borderBottom: '1px solid rgba(255,255,255,0.05)',
      }}
    >
      <KpiCard
        label="Wszystkie elementy"
        value={allElements.length}
        accentColor="#64748b"
        sublabel={`${nodes.length} węzły / ${edges.length} kable`}
      />
      <KpiCard
        label="Sprawne"
        value={counts.operational}
        accentColor="#34d399"
        sublabel="operational"
      />
      <KpiCard
        label="Do sprawdzenia"
        value={counts.investigation}
        accentColor="#fbbf24"
        sublabel="under investigation"
      />
      <KpiCard
        label="Awaria"
        value={counts.fault}
        accentColor="#f87171"
        sublabel="fault / no power"
      />

      {/* Divider */}
      <div
        className="h-10 w-px mx-1 shrink-0"
        style={{ background: 'linear-gradient(to bottom, transparent, #334155, transparent)' }}
      />

      {/* Legend */}
      <div className="flex items-center gap-4 text-[10px] text-slate-500 shrink-0">
        <div className="flex items-center gap-1.5">
          <svg width="24" height="8">
            <line x1="0" y1="4" x2="24" y2="4" stroke="#34d399" strokeWidth="2" />
            <circle cx="16" cy="4" r="3" fill="#34d399" />
          </svg>
          <span>Zasilany</span>
        </div>
        <div className="flex items-center gap-1.5">
          <svg width="24" height="8">
            <line x1="0" y1="4" x2="24" y2="4" stroke="#fbbf24" strokeWidth="2" strokeDasharray="4 2" />
          </svg>
          <span>Do sprawdz.</span>
        </div>
        <div className="flex items-center gap-1.5">
          <svg width="24" height="8">
            <line x1="0" y1="4" x2="24" y2="4" stroke="#f87171" strokeWidth="2" strokeDasharray="4 2" />
          </svg>
          <span>Awaria</span>
        </div>
      </div>
    </div>
  );
}
